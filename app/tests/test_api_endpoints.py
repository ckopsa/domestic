...
from app.db_models.enums import WorkflowStatus, TaskStatus # Imported TaskStatus
from app.models import WorkflowDefinition as WorkflowDefinitionModel, TaskDefinitionBase, WorkflowInstance as WorkflowInstanceModel, TaskInstance as TaskInstanceModel
...
async def test_list_workflow_definitions_with_name_filter(api_client):
    def_alpha_data = {"name": "Test Workflow Alpha", "description": "Alpha test", "task_definitions": [{"name": "Task A", "order": 0}]}
    def_beta_data = {"name": "Another Workflow Beta", "description": "Beta test", "task_definitions": [{"name": "Task B", "order": 0}]}
    def_gamma_data = {"name": "Test Workflow Gamma", "description": "Gamma test", "task_definitions": [{"name": "Task C", "order": 0}]}
...
async def test_create_workflow_definition(api_client):
    data = {"name": "Test Workflow Create", "description": "A test workflow", "task_definitions": [{"name": "Task 1", "order": 0}, {"name": "Task 2", "order": 1}]}
...
async def test_edit_workflow_definition(api_client):
    definition_data = {"name": "Original Workflow Edit", "description": "Original description", "task_definitions": [{"name": "Original Task 1", "order": 0}]}
    ...
    update_data = {"name": "Updated Workflow Edit", "description": "Updated description", "task_definitions": [{"name": "Updated Task A", "order": 0}, {"name": "Updated Task B", "order": 1}]}
...
async def test_edit_workflow_definition_not_found(api_client):
    data = {"name": "Updated Workflow Not Found", "description": "Updated description", "task_definitions": [{"name": "Updated Task 1", "order": 0}]}
...
async def test_delete_workflow_definition(api_client):
    definition_data = {"name": "Workflow to Delete", "description": "This will be deleted", "task_definitions": [{"name": "Task 1", "order": 0}]}
    create_response = api_client.post("/api/workflow-definitions", json=definition_data)
    assert create_response.status_code == 201
    definition_id = create_response.json()["id"]
...
async def test_create_workflow_instance(api_client):
    definition_data = {"name": "Workflow for Instance Create", "description": "Test", "task_definitions": [{"name": "Task 1", "order": 0}]}
    create_response = api_client.post("/api/workflow-definitions", json=definition_data)
    assert create_response.status_code == 201
    definition_id = create_response.json()["id"]
...
async def test_get_workflow_instance(api_client):
    definition_data = {"name": "Workflow for Instance Get", "description": "Test", "task_definitions": [{"name": "Task X", "order": 0}]}
...
async def test_complete_task(api_client):
    definition_data = {"name": "Task Completion Workflow", "description": "Test", "task_definitions": [{"name": "Task 1", "order": 0}]}
    create_def_response = api_client.post("/api/workflow-definitions", json=definition_data)
    definition_id = create_def_response.json()["id"]
    create_inst_response = api_client.post("/api/workflow-instances", json={"definition_id": definition_id})
    instance_id = create_inst_response.json()["id"]
...
async def test_list_user_workflows(api_client):
    definition_data = {"name": "User Workflow Test For List", "description": "Test", "task_definitions": [{"name": "Task 1", "order": 0}]}
    create_def_response = api_client.post("/api/workflow-definitions", json=definition_data)
...
def mock_workflow_service_for_share(monkeypatch):
    ...
    yield mock_service
    if original_get_workflow_service: monkeypatch.setitem(app.dependency_overrides, get_workflow_service, original_get_workflow_service)
    else: monkeypatch.delitem(app.dependency_overrides, get_workflow_service, raising=False)
...
def client_as_user_archive(db_session_fixture, monkeypatch):
    ...
    if original_override_db: monkeypatch.setitem(app.dependency_overrides, get_db, original_override_db)
    else: monkeypatch.delitem(app.dependency_overrides, get_db, raising=False)
    if original_override_user: monkeypatch.setitem(app.dependency_overrides, get_current_active_user, original_override_user)
    else: monkeypatch.delitem(app.dependency_overrides, get_current_active_user, raising=False)
...
def create_test_workflow_instance_for_archive(
    ...
    if status != WorkflowStatus.active:
        ...
        try:
            ...
            instance_json = updated_instance_response.json()["instance"] # The API returns {"instance": ..., "tasks": ...}
        finally:
...
def mock_dependencies_for_my_workflows(monkeypatch):
    ...
    async def mock_render_func(template_name, request_obj, context): return f"Mocked {template_name}" # Simplified
    ...
    for dep, orig_override in original_deps.items():
        if orig_override: monkeypatch.setitem(app.dependency_overrides, dep, orig_override)
        else: monkeypatch.delitem(app.dependency_overrides, dep, raising=False)

@pytest.mark.asyncio
async def test_my_workflows_no_query_parameters(api_client, mock_dependencies_for_my_workflows): # Changed client to api_client
    ...
    assert response.status_code == 200
    mock_service.list_instances_for_user.assert_called_once_with(user_id=mock_user.user_id, created_at_date=date.today(), status=None, definition_id=None) # Expected status=None
...
@pytest.mark.asyncio
async def test_view_shared_workflow_valid_token(api_client, mock_workflow_service_for_share, monkeypatch): # Changed client to api_client
    original_auth_override = app.dependency_overrides.pop(get_current_active_user, None)
    mock_instance_data = MagicMock(spec=WorkflowInstanceModel); mock_instance_data.name = "My Shared Workflow"; mock_instance_data.id = MOCK_INSTANCE_ID_SHARE; mock_instance_data.status = WorkflowStatus.active; mock_instance_data.created_at = date.today()
    mock_tasks_data = [MagicMock(spec=TaskInstanceModel)]
    mock_workflow_service_for_share.get_workflow_instance_by_share_token = AsyncMock(return_value={"instance": mock_instance_data, "tasks": mock_tasks_data})
    mock_renderer_instance = MagicMock(spec=HtmlRendererInterface)
    async def mock_render_func(template_name, request_obj, context): from fastapi.responses import HTMLResponse; return HTMLResponse(f"Mock render of {template_name}.")
    mock_renderer_instance.render = AsyncMock(side_effect=mock_render_func)
    ...
    assert response.status_code == 200
    if original_auth_override: app.dependency_overrides[get_current_active_user] = original_auth_override
    elif get_current_active_user in app.dependency_overrides: del app.dependency_overrides[get_current_active_user]
    if original_renderer_override: app.dependency_overrides[get_html_renderer] = original_renderer_override
    else: monkeypatch.delitem(app.dependency_overrides, get_html_renderer, raising=False)

@pytest.mark.asyncio
async def test_view_shared_workflow_invalid_token(api_client, mock_workflow_service_for_share, monkeypatch): # Changed client to api_client
    original_auth_override = app.dependency_overrides.pop(get_current_active_user, None)
    ...
    async def mock_render_error_func(template_name, request_obj, context): from fastapi.responses import HTMLResponse; return HTMLResponse(f"Mock error page", status_code=404)
...
@pytest.mark.asyncio
async def test_my_workflows_with_created_at(api_client, mock_dependencies_for_my_workflows): # Changed client to api_client
    mock_service, mock_renderer = mock_dependencies_for_my_workflows
    ...
    assert response.status_code == 200
    mock_service.list_instances_for_user.assert_called_once_with(user_id=mock_user.user_id, created_at_date=test_date_obj, status=None, definition_id=None) # Expected status=None
...
async def test_my_workflows_with_status(api_client, mock_dependencies_for_my_workflows): # Changed client to api_client
    ...
    assert response.status_code == 200
    mock_service.list_instances_for_user.assert_called_once_with(user_id=mock_user.user_id, created_at_date=date.today(), status=WorkflowStatus.completed, definition_id=None)

@pytest.mark.asyncio
async def test_my_workflows_with_all_statuses(api_client, mock_dependencies_for_my_workflows): # Changed client to api_client
    mock_service, mock_renderer = mock_dependencies_for_my_workflows
    response = api_client.get("/my-workflows?status=") # Changed client to api_client
    assert response.status_code == 200
    mock_service.list_instances_for_user.assert_called_once_with(user_id=mock_user.user_id, created_at_date=date.today(), status=None, definition_id=None)

@pytest.mark.asyncio
async def test_my_workflows_with_created_at_and_status(api_client, mock_dependencies_for_my_workflows): # Changed client to api_client
    mock_service, mock_renderer = mock_dependencies_for_my_workflows
    test_date_str = "2023-02-20"; test_date_obj = date(2023,2,20)
    response = api_client.get(f"/my-workflows?created_at={test_date_str}&status=active") # Changed client to api_client, and status to active
    assert response.status_code == 200
    mock_service.list_instances_for_user.assert_called_once_with(user_id=mock_user.user_id, created_at_date=test_date_obj, status=WorkflowStatus.active, definition_id=None) # Changed status to active

@pytest.mark.asyncio
async def test_my_workflows_invalid_created_at(api_client, mock_dependencies_for_my_workflows): # Changed client to api_client
    mock_service, mock_renderer = mock_dependencies_for_my_workflows
...
@pytest.mark.asyncio
async def test_my_workflows_invalid_status(api_client, mock_dependencies_for_my_workflows): # Changed client to api_client
    mock_service, mock_renderer = mock_dependencies_for_my_workflows
    response = api_client.get("/my-workflows?status=invalidstatus") # Changed client to api_client
    assert response.status_code == 200
    mock_service.list_instances_for_user.assert_called_once_with(user_id=mock_user.user_id, created_at_date=date.today(), status=WorkflowStatus.active, definition_id=None) # Expect active for invalid status

@pytest.mark.asyncio
async def test_my_workflows_with_definition_id_filter(api_client, mock_dependencies_for_my_workflows): # Changed client to api_client
    mock_service, mock_renderer = mock_dependencies_for_my_workflows
    ...
    assert response.status_code == 200
    mock_service.list_instances_for_user.assert_called_once_with(user_id=mock_user.user_id, created_at_date=date.today(), status=None, definition_id=test_definition_id) # Expected status=None

@pytest.mark.asyncio
async def test_my_workflows_with_definition_id_and_other_filters(api_client, mock_dependencies_for_my_workflows): # Changed client to api_client
    mock_service, mock_renderer = mock_dependencies_for_my_workflows
    test_definition_id = "test_def_id_456"; test_date_str = "2023-03-25"; test_date_obj = date(2023,3,25); test_status = WorkflowStatus.completed; test_status_str = "completed"
    response = api_client.get(f"/my-workflows?definition_id={test_definition_id}&created_at={test_date_str}&status={test_status_str}") # Changed client to api_client
    assert response.status_code == 200
    mock_service.list_instances_for_user.assert_called_once_with(user_id=mock_user.user_id, created_at_date=test_date_obj, status=test_status, definition_id=test_definition_id)
...
async def test_archive_instance_not_authenticated(client_as_user_archive, db_session_fixture, monkeypatch):
    ...
    setup_client = client_as_user_archive(owner)
    instance = create_test_workflow_instance_for_archive(setup_client, owner.user_id, status=WorkflowStatus.active, def_name_suffix="notauth_2")
    instance_id = instance["id"]
...
async def test_archive_instance_other_user(client_as_user_archive, db_session_fixture, monkeypatch):
    ...
    setup_client = client_as_user_archive(owner)
    instance = create_test_workflow_instance_for_archive(setup_client, owner.user_id, status=WorkflowStatus.active, def_name_suffix="otheruser_2")
    instance_id = instance["id"]
...
async def test_archive_instance_already_completed(client_as_user_archive, db_session_fixture, monkeypatch):
    ...
    setup_client = client_as_user_archive(owner)
    instance = create_test_workflow_instance_for_archive(setup_client, owner.user_id, status=WorkflowStatus.completed, def_name_suffix="completed_2")
    instance_id = instance["id"]
    test_action_client = client_as_user_archive(owner)
    response = test_action_client.post(f"/workflow-instances/{instance_id}/archive", follow_redirects=False)
    assert response.status_code == 400
...
async def test_archive_instance_already_archived(client_as_user_archive, db_session_fixture, monkeypatch):
    ...
    setup_client = client_as_user_archive(owner)
    instance = create_test_workflow_instance_for_archive(setup_client, owner.user_id, status=WorkflowStatus.archived, def_name_suffix="alreadyarchived_2")
    instance_id = instance["id"]
    test_action_client = client_as_user_archive(owner)
    response = test_action_client.post(f"/workflow-instances/{instance_id}/archive", follow_redirects=False)
    assert response.status_code == 303
...
async def test_unarchive_instance_success(client_as_user_archive, db_session_fixture, monkeypatch):
    ...
    setup_client = client_as_user_archive(owner)
    instance = create_test_workflow_instance_for_archive(setup_client, owner.user_id, status=WorkflowStatus.archived, def_name_suffix="unarchive_success_2")
    instance_id = instance["id"]
    test_action_client = client_as_user_archive(owner)
...
async def test_unarchive_instance_not_authenticated(client_as_user_archive, db_session_fixture, monkeypatch):
    ...
    setup_client = client_as_user_archive(owner)
    instance = create_test_workflow_instance_for_archive(setup_client, owner.user_id, status=WorkflowStatus.archived, def_name_suffix="unarchive_notauth_2")
    instance_id = instance["id"]
    unauth_client = client_as_user_archive(None)
...
async def test_unarchive_instance_other_user(client_as_user_archive, db_session_fixture, monkeypatch):
    ...
    setup_client = client_as_user_archive(owner)
    instance = create_test_workflow_instance_for_archive(setup_client, owner.user_id, status=WorkflowStatus.archived, def_name_suffix="unarchive_other_user_2")
    instance_id = instance["id"]
    other_user_client = client_as_user_archive(other)
...
async def test_unarchive_instance_not_archived_state_active(client_as_user_archive, db_session_fixture, monkeypatch):
    ...
    setup_client = client_as_user_archive(owner)
    instance = create_test_workflow_instance_for_archive(setup_client, owner.user_id, status=WorkflowStatus.active, def_name_suffix="unarchive_already_active_2")
    instance_id = instance["id"]
    test_action_client = client_as_user_archive(owner)
    response = test_action_client.post(f"/workflow-instances/{instance_id}/unarchive", follow_redirects=False)
    assert response.status_code == 400
...
async def test_unarchive_instance_not_archived_state_completed(client_as_user_archive, db_session_fixture, monkeypatch):
    ...
    setup_client = client_as_user_archive(owner)
    instance = create_test_workflow_instance_for_archive(setup_client, owner.user_id, status=WorkflowStatus.completed, def_name_suffix="unarchive_already_completed_2")
    instance_id = instance["id"]
    test_action_client = client_as_user_archive(owner)
    response = test_action_client.post(f"/workflow-instances/{instance_id}/unarchive", follow_redirects=False)
    assert response.status_code == 400
...
async def test_unarchive_instance_non_existent(client_as_user_archive, db_session_fixture, monkeypatch):
    ...
    test_action_client = client_as_user_archive(owner)
    response = test_action_client.post("/workflow-instances/non_existent_unarchive_id_2/unarchive", follow_redirects=False)
    assert response.status_code == 404
