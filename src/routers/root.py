...
@router.get(
    ...
    openapi_extra={
        "pageTransitions": [
            "home",
            "get_workflow_definitions",
            "list_workflow_definitions_page",
...
@router.get(
    "/cj-workflow-definitions/{definition_id}",
    response_model=CollectionJson,
    openapi_extra={
        "pageTransitions": ["home", "get_workflow_definitions", "view_workflow_definition",
...
@router.post(
    "/cj-workflow-definitions/simpleForm",
    response_model=CollectionJson,
    summary="Create Workflow Definition",
)
async def simple_create_workflow_definition(
    ...
    created_definition = await service.create_new_definition(
        ...
        description=definition.description,
        task_definitions=[
            models.TaskDefinitionBase(name=task_name, order=order, due_datetime_offset_minutes=0)
            for order, task_name in enumerate("\n".split(definition.task_names)) if task_name.strip()
        ]
    )
    return RedirectResponse(
        url=str(request.url_for("view_workflow_definition", definition_id=created_definition.id)),
